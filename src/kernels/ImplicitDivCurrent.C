...
#include "ImplicitDivCurrent.h"

registerMooseObject("zebraApp", ImplicitDivCurrent);

InputParameters
ImplicitDivCurrent::validParams()
{
...
ImplicitDivCurrent::ImplicitDivCurrent(const InputParameters & parameters) : Kernel(parameters),
 _n(coupledValue("n")),
...
Real
ImplicitDivCurrent::computeQpResidual()
{
  return _u[_qp] * _test[_i][_qp] - (1.0/_L[_qp]) * _n[_qp] * _psi[_qp] * _test[_i][_qp];
}

Real
ImplicitDivCurrent::computeQpJacobian()
{
...
Real
ImplicitDivCurrent::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_psi_var == jvar) 
