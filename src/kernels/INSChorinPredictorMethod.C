...
#include "INSChorinPredictorMethod.h"
#include "MooseMesh.h"

registerMooseObject("zebraApp", INSChorinPredictorMethod);

InputParameters
INSChorinPredictorMethod::validParams()
{
...
INSChorinPredictorMethod::INSChorinPredictorMethod(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
INSChorinPredictorMethod::computeQpResidual()
{
  ...
  RealTensorValue grad_U_old(_grad_u_vel_old[_qp], _grad_v_vel_old[_qp], _grad_w_vel_old[_qp]);
  
  Real symmetric_part = (_u[_qp] - U_old(_component)) * _test[_i][_qp];

  // Convective part.  Remember to multiply by _dt!
  Real convective_part = _dt * (grad_U_old * U_old) * test;
...
Real
INSChorinPredictorMethod::computeQpJacobian()
{
...
Real
INSChorinPredictorMethod::computeQpOffDiagJacobian(unsigned jvar)
{
  switch (_predictor_enum)
  {
    case OLD:
